...
export class ConfigStorageProvider implements StorageProvider {
    ...
    async get(key: string): Promise<string | undefined> {
        try {
            ...
            if (value !== undefined) {
                this.logger.debug(`[ConfigStorageProvider] Retrieved value for key: ${key}`);
            }

            return value;
        } catch (error) {
            this.logger.error(`[ConfigStorageProvider] Error retrieving key ${key}:`, error);
            throw new ConfigurationError(`Failed to retrieve configuration value for key: ${key}`, {
                key,
                originalError: error,
            });
        }
    ...
    async delete(key: string): Promise<void> {
        try {
            ...
            this.logger.debug(`[ConfigStorageProvider] Deleted value for key: ${key}`);
        } catch (error) {
            this.logger.error(`[ConfigStorageProvider] Error deleting key ${key}:`, error);
            throw new ConfigurationError(`Failed to delete configuration value for key: ${key}`, {
                key,
                originalError: error,
            });
        }
    ...
    async has(key: string): Promise<boolean> {
        try {
            ...
            this.logger.warning(`[ConfigStorageProvider] Error checking key ${key}:`, error);
            return false;
        }
    }
