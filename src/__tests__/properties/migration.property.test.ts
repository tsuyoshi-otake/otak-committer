                ...
                async ({ openaiKey, githubToken }) => {
                    ...
                    try {
                        ...
                        if (migratedOpenAI !== openaiKey) {
                            console.error(
                                `OpenAI key migration failed: expected ${openaiKey}, got ${migratedOpenAI}`,
                            );
                            return false;
                        }
                        ...
                        if (migratedGitHub !== githubToken) {
                            console.error(
                                `GitHub token migration failed: expected ${githubToken}, got ${migratedGitHub}`,
                            );
                            return false;
                        }

                        // Verification 2: Legacy data should be cleared from Configuration
                        const remainingOpenAI = await configProvider.get(
                            'otakCommitter.openaiApiKey',
                        );
                        const remainingGitHub = await configProvider.get(
                ...
                async ({ openaiKey }) => {
                    ...
                    try {
                        ...
                        if (migratedKey !== openaiKey) {
                            console.error(
                                `Idempotent migration failed: expected ${openaiKey}, got ${migratedKey}`,
                            );
                            return false;
                        }
                ...
                async ({ service, apiKey }) => {
                    ...
                    try {
                        ...
                        if (migratedKey !== apiKey) {
                            console.error(
                                `Migration failed for ${service}: expected ${apiKey}, got ${migratedKey}`,
                            );
                            return false;
                        }
