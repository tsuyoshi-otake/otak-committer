...
suite('Command Context Properties', () => {
                ...
                runPropertyTest(
                    fc.asyncProperty(
                        ...
                        async (args) => {
                            ...
                            assert.ok(logger !== undefined, 'Command should have access to logger');
                            assert.ok(
                            ...
                            assert.ok(command.executeCalled, 'Execute method should be callable');
    ...
    test('Multiple command instances should have independent contexts', () => {
        runPropertyTest(
            fc.property(fc.integer({ min: 2, max: 10 }), (numCommands) => {
                ...
                for (let i = 0; i < commands.length; i++) {
                    ...
                    assert.ok(command.getLogger() !== undefined, `Command ${i} should have logger`);
                    assert.ok(command.getConfig() !== undefined, `Command ${i} should have config`);
                    assert.ok(
