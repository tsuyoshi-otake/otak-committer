    ...
    suite('Property 4: Unicode Preservation', () => {
        ...
        test('Unicode characters should be preserved after sanitization', () => {
            runPropertyTest(
                fc.property(
                    fc
                        .array(unicodeCharArbitrary, { minLength: 1, maxLength: 100 })
                    ...
                    (message) => {
                        ...
                        const originalUnicode = message
                            .split('')
                            .filter((c) => c.charCodeAt(0) > 127 && c.charCodeAt(0) < 0x10000);
    ...
    suite('Property 5: Shell Metacharacter Safety', () => {
        ...
        test('multiple dangerous patterns should all be neutralized', () => {
            runPropertyTest(
                fc.property(
                    fc
                        .array(dangerousPatternArbitrary, { minLength: 1, maxLength: 5 })
            ...
            runPropertyTest(
                fc.property(
                    ...
                    (content) => {
                        ...
                        return sanitized.includes(
                            content.substring(0, Math.min(5, content.length)),
                        );
                    },
                ),
            );
    ...
    suite('Property 7: Typography Normalization', () => {
        ...
        test('all typographic characters should be normalized', () => {
            runPropertyTest(
                fc.property(
                    fc
                        .array(fc.constantFrom(...typographicCharMap.map(([orig]) => orig)), {
                            minLength: 1,
                            maxLength: 10,
                        })
    ...
    suite('Property 8: Control Character Removal', () => {
        ...
        test('control characters should be removed', () => {
            runPropertyTest(
                fc.property(
                    fc
                        .array(controlCharArbitrary, { minLength: 1, maxLength: 10 })
